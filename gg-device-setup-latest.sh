...
# 2. fallback to other locally available python if Python 3.7 is not available and cannot be installed.
...
LATEST_GGC_VERSION_AWARE = "1.11.0"  # TODO : Automatically check the latest version
...
PLATFORM_ARMV6L = "armv6l" # GGC supported for this platform from v1.9.3 and up
SUPPORTED_PLATFORMS = {
    PLATFORM_X86_64,
    PLATFORM_ARMV7L,
    PLATFORM_AARCH64,
...
class ArgsCollection(Step):
    ...
    ARG_MESSAGES = {
        ...
        KEY_GROUP_NAME: "Enter a name for the Greengrass group, or press 'Enter' to use '{}'.".format(
            DEFAULT_GROUP_NAME),
    ...
    def execute(self):
        ...
        ArgsCollection.disable_helloworld_lambda_if_runtime_is_not_python37(self._args)
    ...
    def pre_validation():
        if ArgsCollection.check_whether_ggc_is_running() is True:
            print(
                "GreengrassDeviceSetup has stopped because the Greengrass core software is already running on the device.")
    ...
    def _check_both_exist_access_id_and_secret_key(cls, args):
        if not args.get(KEY_AWS_ACCESS_KEY_ID) or not args.get(KEY_AWS_SECRET_ACCESS_KEY):
    ...
    # Lambda function - helloWorld is only supported if the runtime is python3.7
    @classmethod
    def disable_helloworld_lambda_if_runtime_is_not_python37(cls, args):
        if (sys.version_info.major != 3 or sys.version_info.minor != 7) and args[KEY_HAS_HELLO_WORLD_LAMBDA] is True:
...
class CGroupBootstrap(Step):
    ...
    def run_cgroupfs_mount_script(args):
        ...
        try:
            ...
            download_file_from_url(url=CGROUPFS_MOUNT_SCRIPT_REMOTE_LOCATION,
                                   filepath=CGroupBootstrap.CGROUPFS_MOUNT_SH)
...
class GGCDependencyCheck(Step):
    ...
    def download_checker(args, major_minor_version):
        ...
        download_file_from_url(url=checker_remote_location,
                               filepath=GGC_DEPENDENCY_CHECKER_ZIP_FORMAT.format(major_minor_version))
...
class FunctionDefinitionBootstrap(Step):
    ...
    KEY_ROLE = 'Role'
    KEY_ARN = 'Arn'
    MODIFIED_LAMBDA_HELLO_WORLD_SCRIPT = ""
    FUNCTION_NAME = "Greengrass_HelloWorld_" + Step.get_random_string(5)
    ...
    def execute(self):
        ...
        if self._args[KEY_HAS_HELLO_WORLD_LAMBDA]:  # only do the following if a hello-world lambda is requested
            ...
            try:
                ...
                FunctionDefinitionBootstrap.generate_lambda_function_message()
    ...
    @classmethod
    def generate_lambda_function_message(cls):
        cls.MODIFIED_LAMBDA_HELLO_WORLD_SCRIPT = LAMBDA_HELLO_WORLD_SCRIPT.replace("Greengrass Core",
                                                       "the Lambda function - {}".format(cls.FUNCTION_NAME))
    ...
    def prepare_lambda_execution_role(cls, args, session):
        ...
        region = args[KEY_AWS_REGION]
        iam_client = session.client(IAM)
        lambda_execution_role_name = "GreengrassDeviceSetupLambdaRole_" + Step.get_random_string(5)
        logger.debug(
            "Creating an execution role: {} for the HelloWorld Lambda function.".format(lambda_execution_role_name))
        assume_role_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
    ...
    def prepare_code_package(cls, args):
        ...
        with ChangeDirectory(cls.TMP_DIR):
            ...
            download_file_from_url(url=LATEST_GG_PYTHON_SDK_REMOTE_LOCATION, filepath="./" + cls.GG_PY_SDK_ZIP)
            ...
            with ChangeDirectory("./" + cls.UNZIPPED_GG_PY_SDK_DIR):
                # create gg hello-world lambda
                with open(cls.HELLO_WORLD_PY, "w") as f:
                    f.write(cls.MODIFIED_LAMBDA_HELLO_WORLD_SCRIPT)
...
class CoreSoftwareBootstrap(Step):
    ...
    def download_and_unpack(cls, args):
        ...
        try:
            ...
            download_file_from_url(url=software_url, filepath=cls.CORE_SOFTWARE_FILE)
